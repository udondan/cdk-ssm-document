    ...
    return new Promise(function (resolve, reject) {
        updateDocument(event)
            ...
            .catch(function (err: AWS.AWSError) {
                if (
                    ['InvalidResourceId', 'InvalidDocument'].includes(err.code)
                ) {
                    logger.warn(
        ...
        ssm.updateDocument(
            ...
            function (err: AWS.AWSError, data: AWS.SSM.UpdateDocumentResult) {
                if (err && err.code == 'DuplicateDocumentContent') {
                    ...
                    logger.warn(
                        `Update failed due to ${err.code}. Possibly rollback.`
                    );
                    resolve(event);
                } else if (err) {
                    reject(err);
...
function updateDocumentAddTags(event: Event): Promise<Event | AWS.AWSError> {
    ...
    return new Promise(function (resolve, reject) {
        ...
        ssm.addTagsToResource(
            ...
            function (
                err: AWS.AWSError,
                data: AWS.SSM.AddTagsToResourceResult
            ) {
                if (err) reject(err);
                else resolve(event);
            }
...
function updateDocumentRemoveTags(event: Event): Promise<Event | AWS.AWSError> {
    ...
    return new Promise(function (resolve, reject) {
        ...
        ssm.removeTagsFromResource(
            ...
            function (
                err: AWS.AWSError,
                data: AWS.SSM.RemoveTagsFromResourceResult
            ) {
                if (err) reject(err);
                else resolve(event);
            }
    ...
    return new Promise(function (resolve, reject) {
        if (
            (event.ResourceProperties
                .UpdateDefaultVersion as string).toLowerCase() != 'true'
        ) {
            logger.info(
